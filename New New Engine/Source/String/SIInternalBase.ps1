...
Class SIInternalBase : SceneImage {
    ...
    SIInternalBase(
        ...
        If($(Test-Path $JsonConfigPath) -EQ $true) {
            ...
            #
            # THIS LOGIC HERE IS EXTREMELY BROKEN BECAUSE OF HOW THE COLORS HAVE CHANGED. I'LL NEED TO
            # TEASE THIS OUT.
            [Int]$A = 0
            Foreach($B in $JsonData['ColorData']) {
                If($B -IS [String]) {
                    ...
                    $this.ColorMap[$A] = [ATBackgroundColor24]::new(
                        [TrueColor]::new(
                            [ColorChannel]::new($B[0]),
                            [ColorChannel]::new($B[1]),
                            [ColorChannel]::new($B[2])
                        )
                        # [ConsoleColor24]::new($B[0], $B[1], $B[2])
                    )
                }
                $A++
            }
