...
Class SceneImage {
    ...
    [Void]CreateSceneImageATString(
        [ATBackgroundColor24[]]$ImageColorMap
    ) {
        For($Row = 0; $Row -LT [SceneImage]::Height; $Row++) {
            For($Column = 0; $Column -LT [SceneImage]::Width; $Column++) {
                $RowFlattened = ($Row * [SceneImage]::Width) + $Column
                ...
                $this.Image[$Row, $Column] = [ATSceneImageString]::new(
                    ...
                    [ATCoordinates]::new((5 + $Row), (32 + $Column))
                )
            }
        }
    }
